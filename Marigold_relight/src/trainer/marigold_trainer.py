2# An official reimplemented version of Marigold training script.
...
class MarigoldTrainer:
    def __init__(
        ...
        lr = self.cfg.lr
        self.optimizer = torch.optim.SGD(self.model.unet.parameters(), lr=lr)
        ...
        # Internal variables
        self.epoch = 1
        self.n_batch_in_epoch = 200  # batch index in the epoch, used when resume training
        self.effective_iter = 0  # how many times optimizer.step() is called
        self.in_evaluation = False
        self.global_seed_sequence: List = []  # consistent global seed sequence, used to seed random generator, to ensure consistency when resuming

        self.model,self.optimizer,self.train_loader,self.val_loaders,self.vis_loaders=self.accelerator.prepare(self.model,self.optimizer,self.train_loader,self.val_loaders,self.vis_loaders)
        #self.model.unet = nn.DataParallel(self.model.unet)
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for i,batch in enumerate(self.train_loader):
                ...
                rgb = batch["condition_img_norm"].to(device)
                depth_gt_for_latent = batch["relit_img_norm"].to(device)
                ...
                with torch.no_grad():
                    ...
                    # Encode GT depth
                    gt_depth_latent = self.model.encode_rgb(depth_gt_for_latent)  # [B, 4, h, w]
    ...
    def visualize(self):
        for val_loader in self.vis_loaders:
            ...
            vis_dataset_name = "visualization_dataset"
    ...
    def validate_single_dataset(
        ...
        for i, batch in enumerate(
            ...
            tqdm(data_loader, desc=f"evaluating on validation dataset"),
            start=1,
        ):
            assert 1 == data_loader.batch_size
            # Read input image
            rgb_int = batch["condition_img_int"].squeeze()  # [3, H, W]
            # GT depth

            depth_raw_ts = batch["relit_img_int"].squeeze()
            ...
            for met_func in self.metric_funcs:
                ...
                _metric = met_func(depth_pred_ts.to('cuda:3'), depth_raw_ts.to('cuda:3'),valid_mask=None).item()
            ...
            if save_to_dir is not None:
                ...
                input_img_name = f"input_{img_name}"
                
                png_save_path = os.path.join(save_to_dir, f"{img_name}.png")
                #depth_to_save = (pipe_out.depth_np * 65535.0).astype(np.uint8)
                depth_to_save = (pipe_out.depth_np * 255.0).astype(np.uint8)
                depth_to_save = Image.fromarray(np.transpose(depth_to_save,(1,2,0)))
                # Image.fromarray(depth_to_save).save(png_save_path, mode="I;16")
                depth_to_save.save(png_save_path, mode="I;16")

                png_save_path_input = os.path.join(save_to_dir, f"{input_img_name}.png")
                input_to_save = np.array(rgb_int).astype(np.uint8)
                input_to_save = Image.fromarray(np.transpose(input_to_save,(1,2,0)))
                # Image.fromarray(depth_to_save).save(png_save_path, mode="I;16")
                input_to_save.save(png_save_path_input, mode="I;16")
    ...
    def save_checkpoint(self, ckpt_name, save_train_state):
        ...
        # dir_path_to_remove = './output/train_marigold/checkpoint/old_latest'
        ...
        #         os.path.dirname(ckpt_dir), f"old_{os.path.basename(ckpt_dir)}"
        #     )
