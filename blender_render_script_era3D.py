...
def render_and_save(view_id, env_id, object_uid, len_val, azimuth, elevation, distance,envpath,  ortho=False):
    # print(view_id)
    # render the image
    if os.path.exists(envpath):
        hdri_file = cv2.imread(envpath,cv2.IMREAD_UNCHANGED)
        hdri_file = cv2.resize(hdri_file,(256,256),interpolation = cv2.INTER_LINEAR)
        link_env(scene,envpath,cam)
        #hdri_file = np.clip(hdri_file, 0, 255).astype(np.uint16)
        if os.path.exists(os.path.join(args.output_dir, object_uid,'environments')):
            cv2.imwrite(os.path.join(args.output_dir, object_uid,'environments',f"{env_id}.exr"),hdri_file)
        else:
            os.makedirs(os.path.join(args.output_dir, object_uid, 'environments'),exist_ok=True)
            cv2.imwrite(os.path.join(args.output_dir, object_uid, 'environments',f"{env_id}.exr"),hdri_file)
        
    render_path = os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'image',f"env_{env_id}.png")
    ...
    depth_file_output.base_path = os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}", 'depth')
    normal_file_output.base_path = os.path.join(args.output_dir, object_uid,f"view_{view_id:03d}", 'normal')

    depth_file_output.file_slots[0].path = f"env_{env_id}"
    normal_file_output.file_slots[0].path = f"env_{env_id}"

    if not os.path.exists(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'image',f"env_{env_id}.png")):
        bpy.ops.render.render(write_still=True)

    if os.path.exists(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'depth',f"env_{env_id}0001.exr")):
        os.rename(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'depth',f"env_{env_id}0001.exr"),
                  os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'depth',f"env_{env_id}.exr"))

    if os.path.exists(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'normal', f"env_{env_id}0001.exr")):
        normal = cv2.imread(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'normal', f"env_{env_id}0001.exr"), cv2.IMREAD_UNCHANGED)
        normal_unit16 = (normal * 65535).astype(np.uint16)
        cv2.imwrite(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'normal', f"env_{env_id}.png"), normal_unit16)
        os.remove(os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}",'normal', f"env_{env_id}0001.exr"))
    ...
    #para_path = os.path.join(args.output_dir, object_uid, f"{view_id:03d}", 'camera', f"{env_id}.npy")
    para_path = os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}", f"camera_{view_id:03d}.npy")
    # np.save(RT_path, RT)
    paras = {}
    ...
    plucker_coords = get_plucker_coordinates(bpy.data.objects['Camera'], os.path.join(args.output_dir, object_uid, f"view_{view_id:03d}", f"plucker_{view_id:03d}.txt"))
...
def save_images(object_file: str) -> None:
    ...
    if os.path.exists(os.path.join(args.output_dir, object_uid)): return

    reset_scene()
    load_object(object_file)
    if(check_vertices(20000)): return   
...
#     bproc.init()
    ...
    osroot = 'D:/CJ/MyBlender'
    envroot = osroot + '/data/hdri_1k'
    envlist = []
    for root, dirs, files in os.walk(envroot): 
        for file in files:
            if file.endswith('.exr'):
                envlist.append(root + '/' + file)
    envlist_sampled = random.sample(envlist,5)
    ...
    for i in range(12):
        ...
        for j in envlist_sampled:
            ...
            print(env_name)
            render_and_save(i, env_name, object_uid, -1, azimuth, 0, distance, envpath, ortho=True)
...
def get_ObjList(objroot):
    ObjList = []
    for root, dirs, files in os.walk(objroot):
        for file in files:
            extension = ('.glb', '.stl', '.obj')
            if file.endswith(extension):
                ObjList.append(root + '/' + file)
    ObjList = sorted(ObjList)
    return ObjList

def link_env(scene,path,camera):
    power = 5
    world = bpy.data.worlds.new("world")
    world.use_nodes = True
    output = world.node_tree.nodes.get("World Output")
    background = world.node_tree.nodes.get("Background")
    env1 = world.node_tree.nodes.new(type='ShaderNodeTexEnvironment')
    env1.image = bpy.data.images.load(path)
    texCoord = world.node_tree.nodes.new(type='ShaderNodeTexCoord')
    mapping1 = world.node_tree.nodes.new(type ='ShaderNodeMapping')
    
    world.node_tree.links.new(mapping1.inputs['Vector'], texCoord.outputs['Generated'])
    world.node_tree.links.new(env1.inputs['Vector'], mapping1.outputs['Vector'])
    world.node_tree.links.new(background.inputs['Color'], env1.outputs['Color'])

    # Get the camera's rotation
    camera_rot = camera.rotation_euler

    # Set the mapping node's rotation to the negative of the camera's rotation
    mapping1.inputs['Rotation'].default_value = [-x for x in camera_rot]
    
    scene.world = world

def add_env(scene,envroot):
    envlist = []
    for root, dirs, files in os.walk(envroot):
        for file in files:
            if file.endswith('.exr'):
    ...
    envpath = envlist[randint(0,len(envlist)-1)]
    link_env(scene,envpath)

def get_plucker_coordinates(camera_obj, output_file_path):
    """
    Calculates and returns the Plücker coordinates of a camera object in Blender.
    Args:
        camera_object (bpy.data.Cameras): The Blender camera object.
        output_file_path (str): Path to the output file where coordinates will be saved.
    Returns:
        list: A list containing the 6 Plücker coordinates (l1, l2, l3, m1, m2, m3).
    """
    # Get camera data
    world_matrix = camera_obj.matrix_world

    # Origin in world coordinates
    origin = world_matrix.to_translation()

    # Convert world matrix to Euler rotation and extract direction
    rotation = world_matrix.to_quaternion().to_euler('XYZ')  # Corrected line
    direction = rotation.to_matrix().col[2]  # Extract direction vector

    # Convert to plucker coordinates
    p = origin.cross(direction)
    q = direction

    #return origin, p, q
    
    # Calculate Plücker coordinates
    l1, l2, l3 = origin.x, origin.y, origin.z
    m1, m2, m3 = direction.x, direction.y, direction.z
    plucker_coords = [l1, l2, l3, m1, m2, m3]
    plucker_coords_dict = {
        "origin_x":l1,
        "origin_y":l1,
        "origin_z":l3,
...
def main():
    #try:
        osroot = 'D:/CJ/MyBlender' 
        objroot = osroot + '/data/glbs'
        start_i = time.time()
        # if args.object_path.startswith("http"):
        #     local_path = download_object(args.object_path)
        #     save_images(local_path)
        #else:
        ObjList = get_ObjList(objroot)
        #for i in range(len(ObjList)):
        n1 = 0
        n2 = 2500
        n = n2-n1
        for i in range(n1,n2):
            local_path = ObjList[i]
            save_images(local_path)
        end_i = time.time()
        print("Finished rendering ", n," objects " "in", end_i - start_i, "seconds. \n", f"Output is available in {args.output_dir}")
...
if __name__ == "__main__":
    try:
        osroot = 'D:/CJ/MyBlender' 
        #objroot = osroot + '/data/glbs'
        objroot = "C:/Users/nii_c/.objaverse/hf-objaverse-v1/glbs"
        start_i = time.time()
        # if args.object_path.startswith("http"):
        #     local_path = download_object(args.object_path)
        #     save_images(local_path)
        ...
        for i in range(n1,n2):
            local_path = ObjList[i]
            # P = multiprocessing.Process(target=save_images,args=[local_path])
            # P.start()
            # P.join()
            save_images(local_path)
        end_i = time.time()
